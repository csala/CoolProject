...
test: ## run tests quickly with the default Python
	python -m pytest --basetemp=${ENVTMPDIR} --cov=cool_project

.PHONY: lint
lint: ## check style with flake8 and isort
	flake8 cool_project tests
	isort -c --recursive cool_project tests
...
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find cool_project -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive cool_project
	isort --apply --atomic --recursive cool_project
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source cool_project -m pytest
	coverage report -m
	coverage html
...
docs: clean-docs ## generate Sphinx HTML documentation, including API docs
	sphinx-apidoc --separate --no-toc -o docs/api/ cool_project
	$(MAKE) -C docs html
